...
const Monitor=(props)=>{
    ...
    const option={
        ...
        xAxis: {
            type: 'category',
            boundaryGap: true,
            axisLabel:{interval:props.data?parseInt(props.data.dataX.length/6):25}, // 间隔几个空格显示
            data: props.data?props.data.dataX.map((i,k)=>{
                i=i.slice(5);
                return i.replace(" ","\n");
            }):[]
        },
        ...
        series: props.data?props.data.series_info:[{
            data:[],
            type:"line"
        }]
    };
    var showChange=(e)=>{
        if(e){
            option.toolbox.feature.myTool.show=false
        }
    ...
    var modal_info={
        ...
        footer:null,
        onCancel:showChange(false),
        width:"80%"
    }
    return <div style={{float:"left","width":props.width||"30%","height":"300px","marginRight":props.right||0}}>
    <ReactEchartsCore option={option} style={{"width":"100%","height":"300px"}} echarts={echarts} />
    <Modal {...modal_info} visible={show?true:false}>
    <ReactEchartsCore option={option} style={{"width":"100%","height":"400px"}} echarts={echarts} />
    </Modal>
    </div>
}
